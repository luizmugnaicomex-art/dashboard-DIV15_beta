...
const viewBrokerBtn = document.getElementById('view-broker-btn') as HTMLButtonElement;
...
let currentViewState: 'vessel' | 'po' | 'warehouse' | 'broker' | 'detailed' = 'vessel';
...
const viewColumns = {
    vessel: ['PURCHASE ORDER', 'HBL/AWB', 'TRANSPORTADORA', 'SHIPMENT STATUS', 'INCOTERM', 'MATERIAL TYPE', 'SHIPMENT REFERENCE', 'DATA DE CHEGADA PORTO', 'return empty container date', 'Dias Restantes', 'warehouse', 'AGENTE DE CARGAS INTL'],
    po: ['VESSEL/FLIGHT', 'HBL/AWB', 'TRANSPORTADORA', 'SHIPMENT STATUS', 'INCOTERM', 'MATERIAL TYPE', 'SHIPMENT REFERENCE', 'DATA DE CHEGADA PORTO', 'return empty container date', 'Dias Restantes', 'warehouse', 'AGENTE DE CARGAS INTL'],
    warehouse: ['VESSEL/FLIGHT', 'PURCHASE ORDER', 'HBL/AWB', 'TRANSPORTADORA', 'SHIPMENT STATUS', 'INCOTERM', 'MATERIAL TYPE', 'SHIPMENT REFERENCE', 'DATA DE CHEGADA PORTO', 'return empty container date', 'Dias Restantes', 'AGENTE DE CARGAS INTL'],
    broker: ['VESSEL/FLIGHT', 'PURCHASE ORDER', 'HBL/AWB', 'TRANSPORTADORA', 'SHIPMENT STATUS', 'INCOTERM', 'MATERIAL TYPE', 'SHIPMENT REFERENCE', 'DATA DE CHEGADA PORTO', 'return empty container date', 'Dias Restantes', 'warehouse']
};
...
function initializeApp() {
    ...
    viewBrokerBtn.addEventListener('click', () => setView('broker'));
...
const translations = {
    ...
    'view_by_broker': { pt: 'Análise por Broker', en: 'Analysis by Broker', zh: '按Broker分析' },
    ...
    'chart_title_containers_by_broker': { pt: 'Total de Containers por Broker', en: 'Total Containers by Broker', zh: '各Broker集装箱总数' },
    ...
    'processes': { pt: 'Processos (BLs)', en: 'Processes (BLs)', zh: '流程 (BLs)' },
    ...
    'card_placeholder_no_brokers': { pt: 'Nenhum Broker encontrado para os filtros aplicados.', en: 'No Brokers found for the applied filters.', zh: '未找到符合所应用筛选条件的Broker。' },
    'card_placeholder_try_resetting': { pt: 'Tente limpar os filtros ou carregar um novo arquivo.', en: 'Try clearing the filters or uploading a new file.', zh: '请尝试清除筛选条件 or 上传新文件。' },
        ...
        try {
            ...
            originalData.forEach(row => {
                ...
                if (warehouse.includes('TECON')) {
                    row['warehouse'] = 'TECON';
                } else if (warehouse === 'INTERMARÍTIMA' || warehouse === 'INTERMARITIMA' || warehouse.includes('INTERMARITIMA') || warehouse.includes('INTERMARÍTIMA')) {
                    row['warehouse'] = 'INTERMARITIMA';
                }

                // Normalize broker names
                const broker = String(row['AGENTE DE CARGAS INTL'] || '').toUpperCase().trim();
                if (broker === 'VBLD' || broker === 'VIABILIDADE') {
                    row['AGENTE DE CARGAS INTL'] = 'VIABILIDADE';
                }
            });
...
function getEarliestArrivalDate(shipments: any[]): Date | null {
    return shipments.reduce((earliest: Date | null, shipment: any) => {
        const arrivalDate = excelDateToJSDate(shipment['DATA DE CHEGADA PORTO']);
...
function applyFiltersAndRender() {
    ...
    setTimeout(() => {
        ...
        if (arrivalStart) filteredData = filteredData.filter(row => {
            const arrivalDate = excelDateToJSDate(row['DATA DE CHEGADA PORTO']);
        ...
        if (deadlineStart) filteredData = filteredData.filter(row => {
            const deadlineDate = excelDateToJSDate(row['return empty container date']);
        ...
        if (poSearchTerm) {
            filteredData = filteredData.filter(row => {
                return String(row['PURCHASE ORDER'] || '').toLowerCase().includes(poSearchTerm);
            });
        }
        ...
        if (blSearchTerm) {
            filteredData = filteredData.filter(row => {
                return String(row['HBL/AWB'] || '').toLowerCase().includes(blSearchTerm);
            });
        }
        ...
        if (brokerSearchTerm) {
            filteredData = filteredData.filter(row => {
                return String(row['AGENTE DE CARGAS INTL'] || '').toLowerCase().includes(brokerSearchTerm);
            });
        }
        ...
        if (selectedPOs.length > 0 && !selectedPOs.includes('')) {
            filteredData = filteredData.filter(row => selectedPOs.includes(row['PURCHASE ORDER'] || translate('no_po')));
        }
        ...
        if (selectedVessels.length > 0 && !selectedVessels.includes('')) {
            filteredData = filteredData.filter(row => selectedVessels.includes(row['VESSEL/FLIGHT'] || translate('no_vessel')));
        }
        ...
        if (selectedStatuses.length > 0 && !selectedStatuses.includes('')) {
            filteredData = filteredData.filter(row => selectedStatuses.includes(row['SHIPMENT STATUS'] || translate('no_status')));
        }
        ...
        if (selectedShipmentTypes.length > 0 && !selectedShipmentTypes.includes('')) {
            filteredData = filteredData.filter(row => selectedShipmentTypes.includes(row['INCOTERM'] || translate('no_shipment_type')));
        }
        ...
        if (selectedCargoTypes.length > 0 && !selectedCargoTypes.includes('')) {
            filteredData = filteredData.filter(row => selectedCargoTypes.includes(row['MATERIAL TYPE'] || translate('no_cargo_type')));
        }
        ...
        if (selectedBatches.length > 0 && !selectedBatches.includes('')) {
            filteredData = filteredData.filter(row => selectedBatches.includes(row['SHIPMENT REFERENCE'] || translate('no_batch')));
        }
        ...
        if (selectedBrokers.length > 0 && !selectedBrokers.includes('')) {
            filteredData = filteredData.filter(row => selectedBrokers.includes(row['AGENTE DE CARGAS INTL'] || translate('no_broker')));
        }
        ...
        if (vesselSearchTerm) {
            filteredData = filteredData.filter(row => {
                return String(row['VESSEL/FLIGHT'] || '').toLowerCase().includes(vesselSearchTerm);
            });
        }
        ...
        if (currentViewState === 'vessel') {
            ...
            renderCharts(processedData, filteredData);
        } else if (currentViewState === 'broker') {
            processedData = processDataByBroker(filteredData);
            renderBrokerDashboard(processedData);
            renderCharts(processedData, filteredData);
...
// --- View Switching ---
function setView(view: 'vessel' | 'po' | 'warehouse' | 'broker' | 'detailed') {
    if (currentViewState === view) return;
    currentViewState = view;
    
    const buttons = { vessel: viewVesselBtn, po: viewPoBtn, warehouse: viewWarehouseBtn, broker: viewBrokerBtn, detailed: viewDetailedBtn };
...
/**
 * Calculates the container count for a single shipment row by summing 'QTY CONTAINER' column.
...
function getContainerCount(shipment: any): number {
    const fcl = parseInt(shipment['QTY CONTAINER'], 10) || 0;
    const lcl = 0;
    return fcl + lcl;
}
...
function calculateRisk(shipment: any) {
    let deadline = excelDateToJSDate(shipment['return empty container date']);
    ...
    const isDelivered = (shipment['SHIPMENT STATUS'] || '').toLowerCase().includes('delivered');
...
function processDataByVessel(data: any[]) {
    ...
    const grouped: Record<string, any[]> = data.reduce((acc: Record<string, any[]>, row: any) => {
        const vesselName = row['VESSEL/FLIGHT'] ? String(row['VESSEL/FLIGHT']).trim().toUpperCase() : defaultVesselName;
        
        const name = vesselName; // Composite key
...
function processDataByPO(data: any[]) {
    return processDataGeneric(data, 'PURCHASE ORDER', translate('no_po'));
}

function processDataByWarehouse(data: any[]) {
    return processDataGeneric(data, 'warehouse', translate('no_warehouse'));
}

function processDataByBroker(data: any[]) {
    return processDataGeneric(data, 'AGENTE DE CARGAS INTL', translate('no_broker'));
}
...
function processDataGeneric(data: any[], groupKey: string, defaultName: string) {
    ...
    return Object.entries(grouped).map(([name, shipments]) => {
        ...
        // Calculate unique BL count for "Processes" metric
        const uniqueBLs = new Set(shipments.map(s => s['HBL/AWB']).filter(Boolean));
        const blCount = uniqueBLs.size || shipments.length; // Fallback if no BLs are defined

        return { name, shipments: processedShipments, totalFCL: totalContainers, blCount, overallRisk, riskCounts, earliestArrival };
    }).sort((a, b) => {
        // Sort chronologically by the earliest arrival date in the group.
        const dateA = a.earliestArrival;
        const dateB = b.earliestArrival;
...
function processDataForDetailedView(data: any[]) {
    ...
    const processedVessels = Object.entries(groupedByVessel).map(([vesselName, shipments]) => {
        ...
        shipments.forEach(s => {
            const poNumber = s['PURCHASE ORDER'] || 'N/A';
        ...
        const poGroups = Array.from(poGroupsMap.entries()).map(([poNumber, data]) => {
            ...
            const cargoType = groupShipments[0]?.['MATERIAL TYPE'] || '';
        ...
        poGroups.forEach(group => {
            group.shipments.sort((a, b) => {
                const blA = String(a['HBL/AWB'] || '');
                const blB = String(b['HBL/AWB'] || '');
...
function renderBrokerDashboard(data: any[]) {
    const headers = getVisibleColumns();
    const renderConfig = {
        placeholderTextKey: "card_placeholder_no_brokers",
        cardTitlePrefix: '',
        columns: headers,
        isBrokerView: true
    };
...
function renderDashboard(data: any[], config: any) {
    ...
    data.forEach(item => {
        const card = createDashboardCard(
            ...
            item.shipments,
            item.overallRisk,
            config.isBrokerView ? item.blCount : undefined
        );
...
function createDetailedCard(vesselName: string, poGroups: any[], totalContainers: number) {
    ...
    const headers = ['CARGO TYPE', 'PO QTY', 'PO NUMBER', 'BATCH', 'BL', 'QTY', 'ETA', 'DEADLINE', 'WAREHOUSE', 'SHIPMENT STATUS', 'AGENTE DE CARGAS INTL'];
    const dataKeys: Record<string, string> = {
        'BATCH': 'SHIPMENT REFERENCE',
        'BL': 'HBL/AWB',
        'ETA': 'DATA DE CHEGADA PORTO',
        'DEADLINE': 'return empty container date',
        'SHIPMENT STATUS': 'SHIPMENT STATUS',
        'WAREHOUSE': 'warehouse',
        'AGENTE DE CARGAS INTL': 'AGENTE DE CARGAS INTL'
    };
    ...
    poGroups.forEach((group: any) => {
        group.shipments.forEach((shipment: any, index: number) => {
            ...
            const etaDeadlineWarehouseStatusBrokerCells = ['ETA', 'DEADLINE', 'WAREHOUSE', 'SHIPMENT STATUS', 'AGENTE DE CARGAS INTL'].map(header => {
                 return `<td class="px-3 py-2 whitespace-nowrap">${shipment[dataKeys[header]] || ''}</td>`;
            }).join('');

            shipmentsHtml += `<tr class="border-b text-xs">${cargoTypeHtml}${poQtyHtml}${poNumberHtml}${otherCells}${qtyCell}${etaDeadlineWarehouseStatusBrokerCells}</tr>`;
        });
    });
...
function createDashboardCard(title: string, totalFCL: number, headers: string[], shipments: any[], risk: string, blCount?: number) {
    const card = document.createElement('div');
    ...
    const shipmentsHtml = sortedShipments.map((s: any) => {
        ...
        const rowData = headers.map(header => {
            ...
            if (header === 'Dias Restantes') {
             ...
             if (header === 'return empty container date') {
    ...
    const countDisplayHtml = blCount !== undefined ? `
        <div class="flex gap-4">
            ...
            <div class="text-right border-l pl-4">
                <span class="block text-2xl font-bold text-indigo-600">${blCount}</span>
                <span class="text-sm font-medium text-gray-500">${translate('processes')}</span>
            </div>
        </div>
    ` : `
        <div class="text-right">
...
function renderCharts(processedData: any[], filteredData: any[]) {
    ...
    if (currentViewState === 'vessel') {
        ...
        barChartTitleKey = 'chart_title_containers_by_broker';
    } else { // Warehouse view
    ...
    filteredData.forEach(row => {
        const status = row['SHIPMENT STATUS'] || translate('no_status');
...
function populateStatusFilter(data: any[]) {
    const statuses = [...new Set(data.map(row => row['SHIPMENT STATUS'] || translate('no_status')))].sort();
...
function populatePoFilter(data: any[]) {
    const pos = [...new Set(data.map(row => row['PURCHASE ORDER'] || translate('no_po')))].sort();
...
function populateVesselFilter(data: any[]) {
    const vessels = [...new Set(data.map(row => row['VESSEL/FLIGHT'] || translate('no_vessel')))].sort();
...
function populateShipmentTypeFilter(data: any[]) {
    const shipmentTypes = [...new Set(data.map(row => row['INCOTERM'] || translate('no_shipment_type')))].sort();
...
function populateCargoTypeFilter(data: any[]) {
    const cargoTypes = [...new Set(data.map(row => row['MATERIAL TYPE'] || translate('no_cargo_type')))].sort();
...
function populateBatchFilter(data: any[]) {
    const batches = [...new Set(data.map(row => row['SHIPMENT REFERENCE'] || translate('no_batch')))].sort();
...
function populateBrokerFilter(data: any[]) {
    const brokers = [...new Set(data.map(row => row['AGENTE DE CARGAS INTL'] || translate('no_broker')))].sort();
...
function sortData(data: any[]) {
    return [...data].sort((a, b) => {
        ...
        if (currentSortKey === 'DATA DE CHEGADA PORTO' || currentSortKey === 'return empty container date') {
...
function handleExcelExport() {
    ...
    if (currentViewState === 'detailed') {
            ...
            vesselData.poGroups.forEach(poGroup => {
                poGroup.shipments.forEach((shipment: any) => {
                    dataToExport.push({
                        ...
                        'Shipowner': shipment['TRANSPORTADORA'] || '',
                        'Shipment Type': shipment['INCOTERM'] || '',
                        'Cargo Type': poGroup.cargoType,
                        'PO Number': poGroup.poNumber,
                        'PO Total Qty': poGroup.quantity,
                        'Batch': shipment['SHIPMENT REFERENCE'] || '',
                        'BL': shipment['HBL/AWB'] || '',
                        'BL Qty': getContainerCount(shipment),
                        'ETA': shipment['DATA DE CHEGADA PORTO'] || '',
                        'Deadline': shipment['return empty container date'] || '',
                        'Warehouse': shipment['warehouse'] || '',
                        'Status': shipment['SHIPMENT STATUS'] || '',
                        'Broker': shipment['AGENTE DE CARGAS INTL'] || ''
                    });
                });
            });
        });
        ...
        const detailedSheet = XLSX.utils.json_to_sheet(dataToExport, {
            header: ['Vessel', 'Shipowner', 'Shipment Type', 'Cargo Type', 'PO Number', 'PO Total Qty', 'Batch', 'BL', 'BL Qty', 'ETA', 'Deadline', 'Warehouse', 'Status', 'Broker']
        });
        ...
        const exportHeaders = [
            'VESSEL/FLIGHT', 'PURCHASE ORDER', 'HBL/AWB', 'TRANSPORTADORA', 'SHIPMENT STATUS', 
            'INCOTERM', 'MATERIAL TYPE', 'SHIPMENT REFERENCE', 'DATA DE CHEGADA PORTO', 
            'return empty container date', 'Dias Restantes', 'warehouse', 'CONTAINER QTY', 'AGENTE DE CARGAS INTL'
        ];
...
function renderModalContent() {
    ...
    const baseHeaders = [
        ...
        'SHIPMENT REFERENCE',
        'DATA DE CHEGADA PORTO',
        'return empty container date',
        'Dias Restantes',
        'AGENTE DE CARGAS INTL'
    ];
    
    // Add view-specific columns at the beginning
    if (currentViewState === 'vessel') {
        headers = ['PURCHASE ORDER', ...baseHeaders];
    } else if (currentViewState === 'po') {
        headers = ['VESSEL/FLIGHT', ...baseHeaders];
    } else { // warehouse or broker view
        headers = ['VESSEL/FLIGHT', 'PURCHASE ORDER', ...baseHeaders];
    }
    ...
    const headerDisplayMap: Record<string, string> = {
        ...
        'INCOTERM': 'Tipo de Envio',
        'MATERIAL TYPE': 'Tipo de Mercadoria',
        'QTY_FCL_LCL': 'Qtd. Containers',
        'SHIPMENT REFERENCE': 'Batch',
        'TRANSPORTADORA': 'Transportadora',
        'SHIPMENT STATUS': 'Status',
        'DATA DE CHEGADA PORTO': 'Chegada Real',
        'return empty container date': 'Prazo Free Time',
        'warehouse': 'Armazém',
        'AGENTE DE CARGAS INTL': 'Broker'
    };
    ...
    const shipmentsHtml = sortedShipments.map((shipment: any) => {
        ...
        const rowData = headers.map(header => {
            ...
            if ((header === 'return empty container date' || header === 'DATA DE CHEGADA PORTO') && String(cellContent).startsWith('1899')) {
                cellContent = 'N/A';
            }
            ...
            if (header === 'return empty container date' || header === 'DATA DE CHEGADA PORTO') {
                return `<td class="px-2 py-2 whitespace-nowrap">${cellContent}</td>`;
            }
            if (header === 'HBL/AWB' || header === 'PURCHASE ORDER') {
                return `<td class="px-2 py-2 whitespace-nowrap">${cellContent}</td>`;
            }
            if (header === 'VESSEL/FLIGHT') {
